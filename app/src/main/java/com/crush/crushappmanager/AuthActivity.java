package com.crush.crushappmanager;
...
// [END initialize_auth]
    ...
    private void attemptLogin() {
        ...
        while (phoneNumber.getText().charAt(0) == '0' || phoneNumber.getText().charAt(0) == ' ')
        ...
        if (cancel) {
            ...
            db.collection("employees").whereEqualTo("phone", phone).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isComplete() && task.getResult().size() > 0) {
                        //show loading screen
                        ...
                        new CountDownTimer(60000, 1000) {
                            ...
                            public void onFinish() {
                                timer.setText(0 + " s");
                                resendCode.startAnimation(AnimationUtils.loadAnimation(AuthActivity.this, R.anim.slide_from_right));
                                resendCode.setVisibility(View.VISIBLE);
                            }
                        }.start();
                        //timer ends here
                    } else {
                        Toast.makeText(AuthActivity.this, "Số điện thoại không đúng.", Toast.LENGTH_LONG).show();
                    }
                }
            });
        }
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (task.getException() instanceof FirebaseAuthInvalidCredentialsException) {
                                ...
                                Toast.makeText(AuthActivity.this, "Invalid Verification Code", Toast.LENGTH_LONG).show();
                            }
                        }
                    }
                });
